...
Module::Module(std::string _name) : name(_name)
{
    ...
    // 加入内置函数getarray，第一个参数是数组指针
    {
        Type * intType = IntegerType::getTypeInt();
        const Type * intPtrType = PointerType::get(intType);
        (void) newFunction("getarray", intType, {new FormalParam(const_cast<Type *>(intPtrType), "")}, true);
    }
    // 加入内置函数putarray，第一个参数是数组长度，第二个参数是数组指针
    {
        Type * intType = IntegerType::getTypeInt();
        const Type * intPtrType = PointerType::get(intType);
        (void) newFunction("putarray",
                           VoidType::getType(),
                           {new FormalParam(intType, ""), new FormalParam(const_cast<Type *>(intPtrType), "")},
                           true);
    }
}
