...
/// @brief 数组维度链表节点
///
typedef struct array_dim_node {
    ast_node * expr;              // 维度表达式（如a[10][20]中的10、20）
    struct array_dim_node * next; // 指向下一个维度
} array_dim_node;

///
/// @brief 数组下标链表节点
///
typedef struct array_index_node {
    ast_node * expr;                // 下标表达式（如a[i][j]中的i、j）
    struct array_index_node * next; // 指向下一个下标
} array_index_node;
