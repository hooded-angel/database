...
string getNodeName(ast_node * astnode)
{
    ...
    switch (astnode->node_type) {
        ...
        case ast_operator_type::AST_OP_GREATER:
            nodeName = ">";
            break;
        case ast_operator_type::AST_OP_LESS:
            nodeName = "<";
            break;
...
Agnode_t * genInternalGraphNode(Agraph_t * g, ast_node * astnode)
{
    ...
    if (node != nullptr) {
        ...
        // ====== 显示数组维度或下标 ======
        // 数组定义/初始化：显示每一维的维度表达式
        if ((astnode->node_type == ast_operator_type::AST_OP_ARRAY_DEF ||
             astnode->node_type == ast_operator_type::AST_OP_ARRAY_INIT) &&
            astnode->array_dims) {
            array_dim_node * p = astnode->array_dims;
            while (p) {
                Agnode_t * dim_node = graph_visit_ast_node(g, p->expr);
                if (dim_node) {
                    agedge(g, node, dim_node, (char *) nullptr, 1);
                }
                p = p->next;
            }
        }
        // 数组访问：显示每一维的下标表达式
        if (astnode->node_type == ast_operator_type::AST_OP_ARRAY_ACCESS && astnode->array_indices) {
            array_index_node * p = astnode->array_indices;
            while (p) {
                Agnode_t * idx_node = graph_visit_ast_node(g, p->expr);
                if (idx_node) {
                    agedge(g, node, idx_node, (char *) nullptr, 1);
                }
                p = p->next;
            }
        }
        ...
        // ====== 显示函数形参为数组时的每一维 ======
        if (astnode->node_type == ast_operator_type::AST_OP_FUNC_FORMAL_PARAM && astnode->array_dims) {
            array_dim_node * p = astnode->array_dims;
            while (p) {
                Agnode_t * dim_node = graph_visit_ast_node(g, p->expr);
                if (dim_node) {
