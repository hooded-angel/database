                               ...
                               ast_node * fourth_child)
{
    ...
    if (fourth_child) {
        (void) node->insert_son_node(fourth_child);
    }
...
{
    ast_node * node = new ast_node(ast_operator_type::AST_OP_ARRAY_INIT);
    node->insert_son_node(id_node);
    node->insert_son_node(init);
    node->array_dims = dims;
    return node;
}
...
{
    // 如果expr为nullptr，表示未指定，构造一个特殊的ast_node
    if (!expr) {
        // 用0表示未指定
        ast_node * fake_dim = ast_node::New(digit_int_attr{0, 0});
        expr = fake_dim;
    }
    array_dim_node * new_node = new array_dim_node{expr, nullptr};
    if (!head)
        return new_node;
    array_dim_node * p = head;
    while (p->next)
        p = p->next;
    p->next = new_node;
    return head;
...
{
    array_index_node * new_node = new array_index_node{expr, nullptr};
    if (!head)
        return new_node;
    array_index_node * p = head;
    while (p->next)
        p = p->next;
    p->next = new_node;
    return head;
...
ast_node * create_array_init_list_node(ast_node * first_elem)
{
    ast_node * node = new ast_node(ast_operator_type::AST_OP_ARRAY_INIT);
    if (first_elem)
        node->insert_son_node(first_elem);
    return node;
}
...
ast_node * create_func_param_node(ast_node * type_node, ast_node * id_node, array_dim_node * dims)
{
    ...
    node->type = type_node->type; // 设置类型
    node->insert_son_node(type_node);
    node->insert_son_node(id_node);
    node->array_dims = dims; // 设置数组维度链表
    return node;
}
