...
std::any MiniCCSTVisitor::visitPrimaryExp(MiniCParser::PrimaryExpContext * ctx)
{
	...
	if (ctx->T_DIGIT()) {
		...
		else if (digitText.substr(0, 2) == "0x" || digitText.substr(0, 2) == "0X") {
			// 十六进制
			val = std::stoul(digitText, nullptr, 16);
		}
		else if (digitText[0] == '0') {
			// 八进制（排除单独的0）
			val = std::stoul(digitText, nullptr, 8);
		}
		else {
			// 十进制（非零开头）
			val = std::stoul(digitText, nullptr, 10);
		}
