    ...
    : GlobalValue(_type, _name), builtIn(_builtin)
{
    ...
    if (!isBuiltin()) {
        // 创建Label指令，关联当前函数
        LabelInstruction * entryLabel = new LabelInstruction(this);
        // 将入口Label添加到IR指令序列的头部
        getInterCode().addInst(entryLabel);
    }
}
...
void Function::toString(std::string & str)
{
    ...
    for (auto & param: params) {
        ...
        if (param->getType()->isArrayType()) {
            // 数组形参，首维输出为0
            ArrayType * arrTy = static_cast<ArrayType *>(param->getType());
            param_str += arrTy->getElementType()->toString() + " " + param->getIRName();
            const std::vector<int> & dims = arrTy->getDims();
            for (size_t i = 0; i < dims.size(); ++i) {
                int d = (i == 0) ? 0 : dims[i]; // 首维强制为0
    ...
    for (auto & var: this->varsVector) {
        ...
        if (var->getType()->isArrayType()) {
            // 数组类型，输出元素类型、变量名和维度
            ArrayType * arrTy = static_cast<ArrayType *>(var->getType());
            declareStr += arrTy->getElementType()->toString() + " " + var->getIRName();
            for (int d: arrTy->getDims()) {
                declareStr += "[" + std::to_string(d) + "]";
            }
        } else {
            // 普通类型
            declareStr += var->getType()->toString() + " " + var->getIRName();
        }
        std::string realName = var->getName();
        if (!realName.empty()) {
            // str += " ; " + std::to_string(var->getScopeLevel()) + ":" + realName;
            declareStr += " ; variable: " + realName;
        }
        declareStr += "\n";
        str += declareStr;
    }
    ...
    for (auto & inst: code.getInsts()) {
        if (inst->hasResultValue()) {
            std::string declareStr = "\tdeclare ";
            if (inst->getType() && inst->getType()->isArrayType()) {
                // 数组类型，输出元素类型、变量名和维度
                ArrayType * arrTy = static_cast<ArrayType *>(inst->getType());
                declareStr += arrTy->getElementType()->toString() + " " + inst->getIRName();
                for (int d: arrTy->getDims()) {
                    declareStr += "[" + std::to_string(d) + "]";
                }
            } else {
...
void Function::renameIR()
{
    ...
    for (auto & param: this->params) {
        if (param->getType() == nullptr) {
            std::cerr << "[renameIR] param '" << param->getIRName() << "' type is nullptr!" << std::endl;
        }
    ...
    for (auto & var: this->varsVector) {
        if (var->getType() == nullptr) {
            std::cerr << "[renameIR] var '" << var->getName() << "' type is nullptr!" << std::endl;
        }
    ...
    for (auto inst: this->getInterCode().getInsts()) {
        if (inst->hasResultValue() && inst->getType() == nullptr) {
            std::cerr << "[renameIR] inst type is nullptr! op=" << (int) inst->getOp() << std::endl;
        }
        if (inst->getOp() == IRInstOperator::IRINST_OP_LABEL) {
